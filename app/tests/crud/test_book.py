...
from app.crud.crud_book import (
    ...
    delete_book,
    get_all_books,
    get_book_by_serial_number,
    update_book,
)
from app.models.book import Book, BookBorrowUpdate, BookCreate
from app.tests.utils import create_random_book, random_six_digit_number


def test_create_book(db: Session) -> None:
    serial_number = random_six_digit_number()
    book_create = BookCreate(
        serial_number=serial_number, title="Test Book", author="Test Author"
    )
...
def test_create_book_invalid_serial_number(db: Session) -> None:
    with pytest.raises(ValidationError):
        book_create = BookCreate(
            serial_number="12345", title="Test Book", author="Test Author"
        )
...
def test_get_book_by_serial_number(db: Session) -> None:
    ...
    fetched_book = get_book_by_serial_number(
        session=db, serial_number=book.serial_number
    )
...
def test_borrow_book(db: Session) -> None:
    ...
    book_update = BookBorrowUpdate(
        is_borrowed=True, borrowed_by=borrow_id, borrowed_at=datetime.now()
    )
...
def test_return_book(db: Session) -> None:
    ...
    borrow_id = random_six_digit_number()
    book_update = BookBorrowUpdate(
        is_borrowed=True, borrowed_by=borrow_id, borrowed_at=datetime.utcnow()
    )
    ...
    book_return_update = BookBorrowUpdate(
        is_borrowed=False, borrowed_by=None, borrowed_at=None
    )
    returned_book = update_book(
        session=db, db_book=updated_book, book_update=book_return_update
    )
